...
export function registerCollectionCreateCommand(
	...
	const createCmd = collectionCommand
		...
		.option(
			"--metadata <json>",
			"Metadata for the document collection in JSON format"
		)
		.option(
			"--project <projectId>",
		...
		await withTokenRefresh(async () => {
			...
			try {
				const metadataObj = options.metadata
					...
					: {};
				await apis.documentsApi.documentControllerCreateCollection(
					project.id,
					options.instance,
					{
						name: options.name,
				...
				console.log(
					chalk.green(
						`Document collection "${options.name}" created successfully.`
					)
				);
			} catch (error: any) {
