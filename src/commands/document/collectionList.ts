...
export function registerCollectionListCommand(
	...
	listCmd.action(async options => {
		await withTokenRefresh(async () => {
			...
			try {
				const response: any =
					await apis.documentsApi.documentControllerGetAllCollections(
						project.id,
						options.instance
					);
				const collections = Array.isArray(response.data?.collections)
				...
				if (collections.length === 0) {
					console.log(
						chalk.yellow(
							`No document collections found for instance ${options.instance} in project ${project.name}`
						)
					);
					return;
				}
