...
export function registerCollectionGetCommand(collectionCommand: Command): void {
	const getCmd = collectionCommand
		.command("get")
		.description(
			"Get details of a specific document collection for a specific instance"
		)
		.requiredOption("--instance <instanceId>", "Instance ID")
		.requiredOption(
			"--collection <collectionId>",
			"ID of the document collection to get"
		)
		.option(
			"--project <projectId>",
	...
	getCmd.action(async options => {
		await withTokenRefresh(async () => {
			...
			try {
				const response =
					await apis.documentsApi.documentControllerGetCollectionInfo(
						project.id,
						options.instance,
						options.collection
					);
