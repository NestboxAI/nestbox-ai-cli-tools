...
export function registerCreateCommand(computeCommand: Command): void {
	computeCommand
		...
		.requiredOption(
			"--image <imageId>",
			"Image ID to use for the compute instance"
		)
		.option(
			"--project <projectId>",
		...
		.action(async options => {
			try {
				...
				try {
					...
					if (
						selectedImage.provisioningParameters &&
						selectedImage.provisioningParameters.properties
					) {
						const requiredParams =
							selectedImage.provisioningParameters.required || [];
						...
						for (const [paramName, paramConfig] of Object.entries(
							...
							const paramUI =
								selectedImage.provisioningParametersUI?.[
									paramName
								] || {};
					...
					try {
						...
						console.log(
							chalk.gray(
								"You can check the status with: nestbox compute list"
							)
						);
						console.log(
							chalk.green("Instance created successfully!")
						);
					} catch (createError: any) {
						creationSpinner.fail(
							"Failed to create compute instance"
						);
						if (
