...
async function refreshAuthToken(
	...
	try {
		...
		if (!userCredentials) {
			return {
				success: false,
				error: "Could not find stored credentials",
			};
		}
...
export async function handle401Error(
	...
	if (error.response && error.response.status === 401) {
		...
		console.log(
			chalk.yellow(
				"Authentication token expired. Attempting to refresh..."
			)
		);
		...
		if (refreshResult.success && retryCallback) {
			console.log(
				chalk.green("Token refreshed successfully. Retrying request...")
			);
			...
			try {
				...
				if (retryError.response && retryError.response.status === 401) {
					throw new Error(
						'Authentication failed after token refresh. Please login again using "nestbox login <domain>".'
					);
				}
			...
			// Refresh failed
			throw new Error(
				'Authentication token has expired and automatic refresh failed. Please login again using "nestbox login <domain>".'
			);
		}
	}
